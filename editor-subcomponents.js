 ...
 * 4) Gizmos de transformación (mover/rotar/escalar) para subcomponentes.
 *
 * Implementación:
 ...
 * - Movimiento/rotación/escala aplica el mismo delta local a TODOS los índices únicos seleccionados.
 * - Commit genera acción undo/redo: {type:'subEdit', id, indices:[...], before:[...], after:[...]}
...
export function setupSubcomponents(api) {
  ...
  function applySelectionRotation(obj, axisLocal, angle) {
    if (!hasSelection()) return 0;

    const centerLocal = getSelectionLocalCenter();
    if (!centerLocal) return 0;
    ...
    unique.forEach(i => {
      ...
      v.sub(centerLocal).applyQuaternion(q).add(centerLocal);
    ...
    state.selection.forEach(s => {
      s.centroidLocal.sub(centerLocal).applyQuaternion(q).add(centerLocal);
    });

    refreshHelpers(obj);

    const afterCenterW = getSelectionWorldCenterForObject(obj);
    if (!beforeCenterW || !afterCenterW) return 0;
    return afterCenterW.distanceTo(beforeCenterW);
  }

  function applySelectionScale(obj, axis, scaleFactor) {
    if (!hasSelection()) return 0;
    ...
    if (axis === 'x') scale.set(scaleFactor, 1, 1);
    else if (axis === 'y') scale.set(1, scaleFactor, 1);
    else if (axis === 'z') scale.set(1, 1, scaleFactor);
    else scale.set(scaleFactor, scaleFactor, scaleFactor);
    ...
    unique.forEach(i => {
      const v = new THREE.Vector3(pos.getX(i), pos.getY(i), pos.getZ(i));
      v.sub(centerLocal).multiply(scale).add(centerLocal);
      pos.setXYZ(i, v.x, v.y, v.z);
    });
    ...
    state.selection.forEach(s => {
      s.centroidLocal.sub(centerLocal).multiply(scale).add(centerLocal);
    });
  ...
  function commitSelectionDeltaAsAction(objectId) {
    ...
    if (!state.baseline || state.baseline.id !== objectId) return null;
    ...
    indices.forEach(i => {
      const bX = state.baseline.positions[i * 3 + 0];
      const bY = state.baseline.positions[i * 3 + 1];
      const bZ = state.baseline.positions[i * 3 + 2];

      const aX = pos.getX(i);
      const aY = pos.getY(i);
      const aZ = pos.getZ(i);

      before.push(bX, bY, bZ);
      after.push(aX, aY, aZ);

      if (!changed && (Math.abs(bX - aX) > 1e-6 || Math.abs(bY - aY) > 1e-6 || Math.abs(bZ - aZ) > 1e-6)) {
        changed = true;
      }
    });
  ...
  function applyPositions(obj, indices, values) {
    const pos = obj.geometry.attributes.position;
    for (let i = 0; i < indices.length; i++) {
      const idx = indices[i];
      const base = i * 3;
      pos.setXYZ(idx, values[base], values[base + 1], values[base + 2]);
    }
    pos.needsUpdate = true;
    obj.geometry.computeVertexNormals();
    obj.geometry.computeBoundingBox();
  ...
  function applySubEditForward(action) {
    ...
    if (!obj) return;
    applyPositions(obj, action.indices, action.after);
  }
  function applySubEditInverse(action) {
    const obj = findObjectById(action.id);
    if (!obj) return;
    applyPositions(obj, action.indices, action.before);
  }
