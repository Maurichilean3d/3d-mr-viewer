...
const btnSubMove = document.getElementById('sub-move');
const btnSubRotate = document.getElementById('sub-rotate');
const btnSubScale = document.getElementById('sub-scale');
...
let subTransformMode = 'translate';
...
function applySelectionUI() {
  ...
  if (selectedObject) {
    ...
    if (currentMode === 'select') {
      ...
      if (center) createGizmoFor(subTransformMode, center, selectedObject, 0.5);
      else removeGizmo();
    } else {
...
function setSubTransformMode(mode) {
  subTransformMode = mode;
  [btnSubMove, btnSubRotate, btnSubScale].forEach(btn => btn.classList.remove('active'));
  if (mode === 'translate') btnSubMove.classList.add('active');
  if (mode === 'rotate') btnSubRotate.classList.add('active');
  if (mode === 'scale') btnSubScale.classList.add('active');
...
function getSubRotationAxisLocal(axis) {
  const axisLocal = axisToVec(axis);
  if (currentSpace === 'world' && selectedObject) {
    axisLocal.applyQuaternion(selectedObject.quaternion.clone().invert()).normalize();
  }
  return axisLocal;
}
...
function manipulateByGizmo(x, y) {
  ...
  // subcomponent transforms (select mode)
  if (currentMode === 'select' && subAPI.hasSelection()) {
    if (subTransformMode === 'translate') {
      if (!dragState) return;
    ...
    if (subTransformMode === 'rotate') {
      const rotScale = 0.02;
      const delta = (x - dragStart.x + y - dragStart.y) * rotScale;
      if (axis === 'x' || axis === 'y' || axis === 'z') {
        const axisLocal = getSubRotationAxisLocal(axis);
        subAPI.applySelectionRotation(selectedObject, axisLocal, delta);
      }
      updateGizmoPose();
      dragStart.set(x, y);
      return;
    ...
    if (subTransformMode === 'scale') {
      const scaleSpeed = 0.01;
      const delta = (dragStart.y - y) * scaleSpeed;
      const scaleFactor = Math.max(0.05, 1 + delta);
      subAPI.applySelectionScale(selectedObject, axis, scaleFactor);
      updateGizmoPose();
      dragStart.set(x, y);
      return;
    }
...
function handleDoubleTap(x, y) {
  ...
  if (currentMode === 'select' && selectedObject) {
    ...
    if (changed) {
      const center = subAPI.getSelectionWorldCenter();
      if (center) {
        createGizmoFor(subTransformMode, center, selectedObject, 0.5); // ✅ smaller
    ...
    if (selectedObject && currentGizmo) {
      ...
      if (h) {
        ...
        if (
          ...
          (currentMode === 'select' && subTransformMode === 'translate' && subAPI.hasSelection())
        ) {
...
btnSubMove.addEventListener('click', () => setSubTransformMode('translate'));
btnSubRotate.addEventListener('click', () => setSubTransformMode('rotate'));
btnSubScale.addEventListener('click', () => setSubTransformMode('scale'));
